...
def evaluate_crf_model(encoder, decoder, data, batch_size):
    use_cuda = torch.cuda.is_available()
    ...
    with torch.no_grad():
        for batch in getBatch(batch_size, data):
            ...
            output, hidden_c = encoder(x, x_mask)
            tag_score, intent_score = decoder.predict(hidden_c, output, x_mask)

            # get predicted tags
            tag_truths.append(tag_target.cpu())
            tag_predictions.append(torch.as_tensor(tag_score))
