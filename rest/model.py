...
# TODO:
# what weights should it be initialized to?

# Set forget biases (to 1, this example code sets them differently)
...
class Encoder(nn.Module):
    ...
    def forward(self, input,input_masking):
        ...
        input_masking : B,T (PAD 마스킹한 ByteTensor) PAD masked ByteTensor
        
        <PAD> 제외한 리얼 Context를 다시 만들어서 아웃풋으로
        Recreate the real context except for <PAD> and use it as an output
        """

        print("input_masking.shape", input_masking.shape)
        print("input_masking", input_masking)
        ...
        for i,o in enumerate(output): # B,T,D
            real_length = input_masking[i].data.tolist().count(0) # 실제 길이 actual lengt
...
class Decoder(nn.Module):
    ...
    def Attention(self, hidden, encoder_outputs, encoder_maskings):
        ...
        hidden = hidden.squeeze(0).unsqueeze(2)  # 히든 : (1,배치,차원) -> (배치,차원,1) Hidden: (1, Batch, Dimension) -> (Batch, Dimension, 1)
        ...
        #print("attn_energies.squeeze(1).shape", attn_energies.squeeze(1).shape)
        #print("attn_energies.shape", attn_energies.shape)
        #print("encoder_maskings", encoder_maskings.shape)
    ...
    def forward(self, input, context, encoder_outputs, encoder_maskings, training = True):
        ...
        for i in range(length): # Input_sequence와 Output_sequence의 길이가 같기 때문.. because they are the same length
            ...
            # 그 다음 Context Vector를 Attention으로 계산 # Then calculate the Context Vector as Attention
            context = self.Attention(hidden[0], encoder_outputs, encoder_maskings) 
        # 요고 주의! time-step을 column-wise concat한 후, reshape!! Yogo beware! After concating the time-step column-wise, reshape!!
