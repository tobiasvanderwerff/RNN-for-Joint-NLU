...
from evalutate_biRNN import evaluate
...
def train(config, train_data=None, validation_data=None, test_data=None, 
    ...
    for step in range(config.step_size):
        ...
        for i, batch in enumerate(getBatch(config.batch_size,train_data)):
            ...
            if i % 100==0:
                ...
                if validation_data:
                    eval_losses, f1_tag_score, intent_accuracy = evaluate(biRNN, validation_data, config.batch_size)
    ...
    _, test_f1_tag_score, test_intent_accuracy = evaluate(biRNN, test_data, config.batch_size)
